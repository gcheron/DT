#! /bin/bash
# cut features into HOG, HOF MBHx and MBHy descriptors
# parameter 1: video list path
# parameter 2: dataset result path
# Example:
# sh generate_jobs_cut.sh /sequoia/data2/gcheron/dummy/videos/video_list.txt /sequoia/data2/gcheron/CVPR14/features/dummy

VIDEOLISTPATH=$1
JOBPATH=$2/jobs/qsub_jobs
JOBLISTNAME=$2/jobs/joblist_cutting.txt
JOBEXT=_cutting.pbs
LOGPATH=$2/jobs/logs/cutting
CUTDIR=$2/cutting
...
do
	...
	vidwe=${vid%.avi}
        ...
        i=$((${#DESC[@]}-1))
        desc=${DESC[i]}
	featres=$CUTDIR/$vidwe.features.${desc}
        if [ -a $featres ]
        then
                continue
        fi
	...
	for ((i = 0; i < ${#DESC[@]}; i++))
		...
		eval "end=\$${desc}_end"
		echo "cat '${FEATDIR}/$vidwe.features' | cut -d$'\t' -f ${begin}-${end} > '$featres'"
	done
